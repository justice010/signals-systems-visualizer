    ...
    const loadChapters = async () => {
      ...
      for (const path in knowledgeModules) {
        try {
          ...
          const firstPara = lines.find(line => line.trim() && !line.startsWith('#') && !line.startsWith('---')) || '阅读该章节的理论详情...';
          const desc = firstPara.length > 80 ? firstPara.substring(0, 77) + '...' : firstPara;
    ...
    <div className="h-full w-full overflow-y-auto bg-gray-900 text-gray-100 p-6 md:p-16 scroll-smooth">
      <header className="max-w-6xl mx-auto mb-10 md:mb-16">
        ...
        <p className="text-lg text-gray-400 font-medium border-l-4 border-blue-500 pl-4 max-w-2xl">
          动态知识库已就绪。系统会自动扫描 <code className="bg-gray-800 px-2 py-1 rounded text-blue-400 text-sm">src/content/knowledge/</code> 下的所有 Markdown 文件并实时更新目录。
        </p>
      </header>
